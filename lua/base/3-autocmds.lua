...
autocmd("FileType", {
  ...
  pattern = "java",
  callback = function() require("base.utils.jdtls").setup() end,
})
...
-- 12. Auto-header on new file creation
autocmd("BufNewFile", {
  pattern = "*",
  callback = function()
    ...
    local ts = os.date("%H:%M %d-%m-%Y")
    local author = "Created by Jonathan"
    local header = string.format("%s %s at %s", prefix, author, ts)
...
vim.api.nvim_create_autocmd("FileType", {
  callback = function()
    ...
    local function italic(group)
      local current = hl(0, { name = group, link = false }) or {}
      current.italic = true
      set(0, group, current)
    end

    local function normal(group)
      local current = hl(0, { name = group, link = false }) or {}
    ...
    -- Appliquer l’italique uniquement à ces groupes
    ...
    for _, group in ipairs(non_italic_groups) do
      normal(group)
    end
  end,
})
...
vim.api.nvim_create_user_command("CopyCursorDiagnostic", function()
  ...
  if diagnostics and #diagnostics > 0 then
    ...
    vim.fn.setreg("+", msg)
    print("Diagnostic copié : " .. msg)
  else
    print("Aucun diagnostic sous le curseur.")
  end
end, {})
